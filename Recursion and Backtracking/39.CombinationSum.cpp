...
public:
    ...
    void find(int index,int target,vector<int> &bucket,vector<int> candidates,vector<vector<int>> &res){
        ...
        if(candidates[index]<=target){// this condition is because if the element is greater than the desired target then its of no use bcuz it can never fulfill teh desired target=0
